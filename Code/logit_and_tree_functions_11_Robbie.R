...
cluster_df <- data.frame(child_age_old, child_female, child_disabled, child_behavior_prob, child_neglect,
                         ...
                         family_setting_trial, family_struct_married, family_struct_unmarried, family_struct_sing_female,
...
logit = function(child_cluster_df)
{
  ...
  x_train_family = subset(train, select=c(13:31))
  x_test_family = subset(test, select=c(13:31))
...
################################
## create jpegs for each tree ##
################################

treeOutput = function(foster_tree, tree_name)
{
...
run_logit = function(cluster_df)
{
  ...
  for (j in child_columns){
    ...
    # treeOutput(temp_tree, name_list[j])
  }
  ...
  colnames(overall_df) = c("name_list=", "error_list", "model_list", "tree_error_list", "tree_list")
